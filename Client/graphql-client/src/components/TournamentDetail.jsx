...
function TournamentDetail() {
  ...
  // Handler for quitting the tournament
  const handleRemovePlayer = async (playerToRemoveId) => {
    try {
      await removeFromTournament({
        variables: { playerId: playerToRemoveId, tournamentId: id },
      });
      setSearchUsername("");
      await searchUser({
        variables: { username: "" },
    ...
    <div className="tournament-card">
      ...
      <ul>
        {tournament.players.map((player) => (
          <li key={player.id} className="user-in-tournament">            
              {player.user?.username || "Deleted User"}
              {user && user.role === "Admin" && (
                <button onClick={() => handleRemovePlayer(player.id)} className="user-in-tournament">
                  Remove
                </button>
              )}            
          </li>
                    ...
                    return (
                      <li key={player.id}>
                        ...
                        {!isPlayerInTournament && (
                          <button onClick={() => handleAddPlayer(player.id)} className="user-in-tournament">
                            Add to Tournament
                          </button>
                        )}
                      </li>
